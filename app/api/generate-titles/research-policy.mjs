import { explicitPublicYouTubeChannelName } from "./guards.mjs";
...
const CHANNEL_RESEARCH_CUE = /\b(?:my|our)\s+(?:(?:(?:last|latest|newest|recent|most\s+recent)\s+)?(?:\d+\s+)?)?(?:youtube\s+)?(?:channel|analytics|stats?|metrics?|uploads?|videos?|performance|audience)|based\s+on\s+(?:my|our)\s+channel|use\s+(?:my|our)\s+channel\b/i;
...
export function resolveResearchAccess(currentMessage, hasYouTubeAttachment = false) {
  ...
  const publicSearch = PUBLIC_RESEARCH_CUE.test(message)
    || Boolean(explicitPublicYouTubeChannelName(message))
    || /(?:https?:\/\/)?(?:www\.)?youtube\.com\/(?:channel\/UC[A-Za-z0-9_-]{20,30}|@[A-Za-z0-9._-]{3,30})\b/i.test(message);
...
const PUBLIC_CREATOR_CONTINUATION_CUE = /\b(?:him|her|them|his|their|hers|that\s+(?:creator|youtuber|channel)|the\s+(?:creator|youtuber|channel)|same\s+(?:creator|youtuber|channel)|based\s+on\s+(?:him|her|them|his|their))\b/i;

export function resolveConversationPublicYouTubeChannel(messages, currentMessage) {
  const directTarget = explicitPublicYouTubeChannelName(currentMessage);
  if (directTarget) return directTarget;
  const message = typeof currentMessage === "string" ? currentMessage : "";
  if (!PUBLIC_CREATOR_CONTINUATION_CUE.test(message) || !Array.isArray(messages)) return "";

  const lastTurn = messages.at(-1);
  const startsAt = lastTurn?.content === message ? messages.length - 2 : messages.length - 1;
  for (let index = startsAt; index >= 0; index -= 1) {
    const turn = messages[index];
    if (!turn || turn.role !== "user") continue;
    const target = explicitPublicYouTubeChannelName(turn.content);
