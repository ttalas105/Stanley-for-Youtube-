...
const twin = {
  ...
  differences: [{ category: "Opening", detail: "Makes the physical constraint visible sooner", twin: "First five seconds", you: "First thirty seconds" }],
...
test("keeps dashboard evidence visible and turns a Creator Twin into a video brief", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/youtube/analytics?**", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
    ...
    current: { views: 8_200_000, watchMinutes: 2_460_000, subscribersGained: 42_000, subscribersLost: 3_200, averageViewDuration: 760, averageViewPercentage: 54 },
    comparison: { views: 7_100_000, watchMinutes: 2_100_000, subscribersGained: 35_000, subscribersLost: 3_000, averageViewDuration: 720, averageViewPercentage: 51 },
    ...
    videos: videos.map((video, index) => ({ id: video.id, views: video.views, watchMinutes: 410_000 - index * 30_000, averageViewDuration: 760 - index * 25, averageViewPercentage: 57 - index * 2, netSubscribers: 8_400 - index * 900, likes: 164_000 - index * 19_000, comments: 7_800 - index * 720, shares: 4_100 - index * 430, commentRate: .28 - index * .02, interactionRate: 6.3 - index * .45 })),
    traffic: [{ source: "YT_SEARCH", views: 2_900_000, watchMinutes: 800_000 }],
    comparisonTraffic: [{ source: "YT_SEARCH", views: 3_600_000, watchMinutes: 940_000 }],
  ...
  await expect(page.getByRole("region", { name: "Channel overview" })).toContainText("8.2M");
  await expect(page.getByRole("button", { name: "30D" })).toHaveAttribute("aria-pressed", "true");
  await expect(page.getByRole("region", { name: "Performance over time" })).toBeVisible();
  const topVideos = page.getByRole("region", { name: "Top videos" });
  await expect(topVideos.getByRole("article")).toHaveCount(5);
  await expect(topVideos.getByRole("button", { name: "Make a video" }).first()).toBeVisible();
  await page.getByRole("tab", { name: "Period analytics" }).click();
  await expect(page.getByRole("region", { name: "Views, current 30 days compared with previous 30 days" })).toBeVisible();
  await page.getByRole("button", { name: "Generate report" }).click();
  await expect(page.getByRole("heading", { name: "Channel performance report" })).toBeVisible();
  ...
  await expect(page.getByLabel("Signals used for matching")).toContainText("TopicsFormatMomentum");
  await page.getByRole("button", { name: "Find my Creator Twin" }).click();
  await expect(page.getByRole("heading", { name: "Jesse James West", exact: true })).toBeVisible({ timeout: 7_000 });
  await expect(page.getByLabel("89% pattern match")).toBeVisible();
  await expect(page.getByRole("heading", { name: "Why Stanley chose Jesse James West" })).toBeVisible();
  await page.getByRole("tab", { name: "Key differences" }).click();
  await expect(page.getByRole("table", { name: "Differences between Jesse James West and your channel" })).toBeVisible();
  await page.getByRole("tab", { name: "Top videos (4)" }).click();
  await expect(page.getByRole("button", { name: "Study video" })).toHaveCount(4);
  ...
  await expect(page.getByLabel("Message Stanley")).toHaveValue(/Visible challenge plus a measurable constraint/);
